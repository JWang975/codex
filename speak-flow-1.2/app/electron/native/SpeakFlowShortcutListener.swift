...
final class ShortcutListener {
    ...
    private var activeModifierShortcuts: [String: String] = [:]
    private var activeModifierDisplays: [String: String] = [:]
    ...
    func run(timeoutMs: Int) {
        ...
        guard let createdTap = CGEvent.tapCreate(
            ...
            emit([
                "type": "status",
                "status": "error",
                "message": "无法启动键盘监听，请在 macOS 隐私设置中允许 Speak flow 使用输入监控；如仍失败，再允许辅助功能。",
            ])
            exit(2)
        }
    ...
    private func handleKeyDown(_ event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        if captureMode {
            let modifiers = activeModifierShortcutsInOrder()
            let shortcut = (modifiers + [key.shortcut]).joined(separator: "+")
            let display = activeModifierDisplaysInOrder() + [key.display]
            emit([
                "type": "capture",
                "shortcut": shortcut,
                "display": display,
    ...
    private func handleFlagsChanged(_ event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        if isDown {
            activeModifierShortcuts[modifier.generic] = modifier.shortcut
            activeModifierDisplays[modifier.generic] = modifier.display
        } else {
            activeModifierShortcuts.removeValue(forKey: modifier.generic)
            activeModifierDisplays.removeValue(forKey: modifier.generic)
        }
        guard isDown else {
            return Unmanaged.passUnretained(event)
        }
        ...
        if captureMode {
            ...
            let workItem = DispatchWorkItem { [weak self] in
                ...
                let shortcuts = self.activeModifierShortcutsInOrder()
                let displays = self.activeModifierDisplaysInOrder()
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(620), execute: workItem)
            return Unmanaged.passUnretained(event)
        }
        ...
        if Self.flags(event.flags, contain: spec.modifiers)
            && (spec.key == modifier.shortcut || spec.key == modifier.generic) {
            trigger(spec.raw)
        }
        return Unmanaged.passUnretained(event)
    }

    private func activeModifierShortcutsInOrder() -> [String] {
        Self.modifierOrder.compactMap { activeModifierShortcuts[$0] }
    }

    private func activeModifierDisplaysInOrder() -> [String] {
        Self.modifierOrder.compactMap { activeModifierDisplays[$0] }
    }
    ...
    private static func requiresNativeBackend(_ shortcut: String) -> Bool {
        ...
        return modifierName(key) != nil
    }

    private static let modifierOrder = ["Control", "Alt", "Shift", "Command", "Fn"]
