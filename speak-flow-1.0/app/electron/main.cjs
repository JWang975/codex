...
let shortcutHelperStdout = "";
let shortcutHelperConfigKey = "";
let shortcutHelperStopping = false;
...
function deepMergeSettings(base, incoming) {
  ...
  next.llm = mergeProtectedConfig(base.llm, patch.llm, [
    "apiKey",
    "baseUrl",
    "baseUrlOpenAI",
    "baseUrlAnthropic",
  ...
  if (patch.shortcutDefaultMigrated !== true && isLegacyDefaultShortcut(next)) {
    next.shortcut = "Fn";
    next.shortcutDisplay = ["Fn"];
    next.shortcutBackend = "native";
    next.shortcutDefaultMigrated = true;
  }
  ...
  if (!next.llm.baseUrl) {
    next.llm.baseUrl =
      next.llm.baseUrlOpenAI ||
      next.llm.baseUrlAnthropic ||
      base.llm.baseUrl ||
      "";
  }
  return next;
...
function isLegacyDefaultShortcut(settings) {
  const shortcut = String(settings?.shortcut || "").replace(/\s+/g, "");
  const display = Array.isArray(settings?.shortcutDisplay) ? settings.shortcutDisplay.join("+") : "";
  return shortcut === "Control+Space" && (!display || display === "Left Control+Space");
}
...
function requiresNativeShortcut(shortcut) {
  ...
  return parts.length === 1 && /^(Left|Right)?(Control|Ctrl|Alt|Option|Shift|Command|Cmd)$/i.test(key);
}
...
function handleShortcutHelperLine(line) {
...
function stopShortcutHelper(disabledStatus) {
  if (shortcutHelperProcess) {
    shortcutHelperStopping = true;
    shortcutHelperProcess.kill("SIGTERM");
    shortcutHelperProcess = null;
  }
  shortcutHelperStdout = "";
  shortcutHelperConfigKey = "";
  if (disabledStatus) {
    setShortcutStatus({
      status: "disabled",
      backend: "native",
...
function startShortcutHelper(settings, reason = "native 快捷键监听已启动。") {
  const helperPath = shortcutHelperPath();
  const shortcut = normalizeNativeShortcut(settings.shortcut);
  const configKey = JSON.stringify({ shortcut, backend: "native" });
  ...
  if (!fs.existsSync(helperPath)) {
    ...
    setShortcutStatus({
      ...
      backend: "native",
      shortcut,
      message: `快捷键 helper 缺失：${helperPath}`,
      pid: undefined,
    });
    return;
  }
  if (shortcutHelperProcess && shortcutHelperConfigKey === configKey) return;
  stopShortcutHelper();
  ...
  shortcutHelperProcess = child;
  shortcutHelperConfigKey = configKey;
  setShortcutStatus({
    status: "starting",
    backend: "native",
    shortcut,
  ...
  child.stdout.on("data", (chunk) => {
    shortcutHelperStdout += chunk;
    const lines = shortcutHelperStdout.split(/\r?\n/);
    shortcutHelperStdout = lines.pop() || "";
    lines.forEach(handleShortcutHelperLine);
  });
  ...
  child.on("error", (error) => {
    if (shortcutHelperProcess === child) shortcutHelperProcess = null;
    setShortcutStatus({
      status: "error",
      backend: "native",
      shortcut,
      message: `无法启动快捷键 helper：${error.message}`,
      pid: undefined,
    });
  });
  ...
  child.on("close", (code, signal) => {
    if (shortcutHelperProcess === child) shortcutHelperProcess = null;
    if (shortcutHelperStopping) {
      shortcutHelperStopping = false;
      return;
    }
    setShortcutStatus({
      ...
      backend: "native",
      shortcut,
      message: `快捷键 helper 已退出${signal ? `：${signal}` : `，代码 ${code}`}。`,
      pid: undefined,
    });
  });
}
...
function refreshShortcut() {
  const settings = loadSettings();
  if (normalizeShortcutBackend(settings.shortcut, settings.shortcutBackend) === "native") {
    stopShortcutHelper();
  }
  registerShortcut();
      ...
      submenu: [
        {
          ...
          enabled: !isBusy,
          click: () => mainWindow?.webContents.send(isActive ? "recording:stop" : "recording:start"),
        },
...
function registerShortcut() {
  ...
  const settings = loadSettings();
  if (normalizeShortcutBackend(settings.shortcut, settings.shortcutBackend) === "native") {
    startShortcutHelper(settings);
    return;
  }

  stopShortcutHelper();
  const shortcut = normalizeElectronShortcut(settings.shortcut);
  ...
  if (!ok) {
    if (!requiresNativeShortcut(settings.shortcut)) {
      startShortcutHelper({ ...settings, shortcut }, "Electron 快捷键不可用，已尝试 native 监听。");
    } else {
      setShortcutStatus({
        status: "error",
        backend: "electron",
        shortcut,
        message: `无法注册快捷键：${settings.shortcut}`,
        pid: undefined,
      });
      showNotification(PORTABLE_APP_NAME, `无法注册快捷键：${settings.shortcut}`);
    }
    return;
  }
  setShortcutStatus({
...
function getLLMBaseUrl(config) {
  return String(config.baseUrl || config.baseUrlOpenAI || config.baseUrlAnthropic || "").replace(/\/$/, "");
}

function inferLLMFormat(config) {
  const baseUrl = getLLMBaseUrl(config).toLowerCase();
  if (config.provider === "anthropic" || baseUrl.includes("anthropic.com") || baseUrl.endsWith("/anthropic")) {
...
async function callLLM(config, messages, signal) {
  ...
  if (apiFormat === "openai_chat_completions") {
    ...
    if (!response.ok) {
      ...
      throw new Error(`智能整理失败：${response.status} ${response.statusText} ${body}`);
    }
...
function installIpcHandlers() {
  ...
  ipcMain.handle("llm:test", async () => {
    ...
    return { ok: result.trim().toLowerCase().includes("ok"), text: result };
  });
  ...
  ipcMain.on("voice-overlay:cancel", () => mainWindow?.webContents.send("recording:cancel"));
  ipcMain.on("voice-overlay:confirm", () => mainWindow?.webContents.send("recording:stop"));
}
